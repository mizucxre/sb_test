...
def add_order(order: Dict[str, Any] = None, **kwargs) -> None:
    ...
    now = _now()
    if df.empty:
        df = pd.DataFrame([{k: "" for k in ["order_id", "client_name", "phone", "origin", "status", "note", "country", "updated_at"]}])
        df.loc[0, ["order_id", "client_name", "phone", "origin", "status", "note", "country", "updated_at"]] = [
            data.get("order_id"), data.get("client_name", ""), data.get("phone", ""), data.get("origin", ""),
            data.get("status", ""), data.get("note", ""), data.get("country", ""), now
        ]
    else:
...
def upsert_address(
    ...
    if df.empty:
        df = pd.DataFrame([{k: "" for k in ["user_id", "username", "full_name", "phone", "city", "address", "postcode", "created_at", "updated_at"]}])
        df.loc[0, ["user_id", "username", "full_name", "phone", "city", "address", "postcode", "created_at", "updated_at"]] = [
            user_id, uname, full_name, phone, city, address, postcode, now, now
        ]
    else:
...
def get_clients_by_usernames(usernames: List[str]) -> List[Dict[str, Any]]:
    ...
    for uname in wanted:
        ...
        if not s.empty:
            ...
            row.pop("username_norm", None)
            row.pop("phone_digits", None)
            result.append(row)
    return result

def export_clients_dataframe(usernames: Optional[List[str]] = None) -> pd.DataFrame:
    df = _get_clients_df()
    if df.empty:
        return df
    if usernames:
...
def subscribe(user_id: int, order_id: str) -> None:
    ...
    now = _now()
    if df.empty:
        df = pd.DataFrame([{k: "" for k in ["user_id", "order_id", "last_sent_status", "created_at", "updated_at"]}])
        df.loc[0, ["user_id", "order_id", "last_sent_status", "created_at", "updated_at"]] = [user_id, order_id, "", now, now]
    else:
...
def set_last_sent_status(user_id: int, order_id: str, status: str) -> None:
    ...
    if df.empty:
        df = pd.DataFrame([{k: "" for k in ["user_id", "order_id", "last_sent_status", "created_at", "updated_at"]}])
        df.loc[0, ["user_id", "order_id", "last_sent_status", "created_at", "updated_at"]] = [user_id, order_id, status, now, now]
    else:
        mask = (df["user_id"].astype(str) == str(user_id)) & (df["order_id"].astype(str).str.lower() == order_id.lower())
        if mask.any():
...
def get_orders_by_phone(phone: str) -> List[Dict[str, Any]]:
    ...
    ws_parts = get_worksheet("participants")
    parts = ws_parts.get_all_records()
    wanted = {_normalize_username(u) for u in usernames}
    oids: List[str] = []
    for r in parts:
        if str(r.get("username", "")).strip().lower() in wanted:
            oid = str(r.get("order_id", "")).strip()
            if oid:
                oids.append(oid)
    if not oids:
